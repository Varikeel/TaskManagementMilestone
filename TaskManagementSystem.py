...
def voice_input():
    """Handles voice input and recognition"""
    recognizer = sr.Recognizer()
    # Flag to hold input
    is_correct = False
    use_mic = messagebox.askyesno(
        "Microphone Confirmation", "This option will take input from your microphone and connect it to Google web services. Would you like to continue?")
    if not use_mic:
        return
    # To repeat for false input
    while not is_correct:
        with sr.Microphone() as source:
            try:
                # Task title
                speak("What would you like the title of the task to be?")
                audio = recognizer.listen(source)
                task_title = recognizer.recognize_google(audio)
                # Ask for confirmation
                speak(f"I heard you say: {task_title}? Is this correct?")
                audio = recognizer.listen(source)
                response = recognizer.recognize_google(audio).lower()
                if "yes" in response:
                    # Ask for a note
                    speak("Awesome! Would you like to add a note?")
                    audio = recognizer.listen(source)
                    note_response = recognizer.recognize_google(audio).lower()
                    if "yes" in note_response:
                        speak("Please provide it!")
                        audio = recognizer.listen(source)
                        task_note = recognizer.recognize_google(audio)
                        speak("Got it!. Task added successfully.")
                    else:
                        task_note = ""
                        speak("Task added successfully.")
                    # Create a new task
                    tasks.append((task_title, "", task_note, False))
                    save_tasks()
                    update_task_list(tasks)
                    # Exit flag loop
                    is_correct = True
                    ...
                    speak(
                        "I didn't understand your response. Please say 'yes' or 'no'.")
            # Errors
            except sr.UnknownValueError:
                speak("Sorry, I couldn't understand your input.")
...
voice_input_button = tk.Button(
    root, text="🎙️", font=("Arial", 20), command=voice_input)
voice_input_button.grid(row=3, column=3, padx=20, pady=20, sticky="se")
